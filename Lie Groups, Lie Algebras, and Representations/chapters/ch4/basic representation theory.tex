...
大部分李群或者李代数都不具有完全可约性。但是，许多我们感兴趣的李群和李代数
都具有完全可约性。

\begin{example}
  令 $\Pi:\mathbb{R}\to \GL(2,\mathbb{C})$ 为
  \[
    \Pi(x)=\begin{pmatrix}
      1 & x \\
      0 & 1
...
\end{example}
\begin{proof}
  设 $\{e_1,e_2\}$ 是 $\mathbb{C}^2$ 的标准基，显然 $e_1$ 张成的子空间
  是 $\Pi$ 的不变子空间。我们断言 $\langle e_1\rangle$ 是唯一的非平凡不变子空间，
  从而说明 $\Pi$ 不是完全可约的。假设 $V$ 是一个非零不变子空间并且 $V\neq\langle e_1\rangle$，取
  $v=ae_1+be_2\in V$，那么 $b\neq 0$，所以
  \[
    be_1=\Pi(1)v-v\in V,
  \]
...
\begin{proposition}
  如果 $V$ 是李群或者李代数的完全可约表示，那么下面的性质成立。
  \begin{enumerate}
    \item 对于 $V$ 的每个不变子空间 $U$，存在另一个不变子空间 $W$，使得
    $V=U\oplus W$。
    \item $V$ 的每个不变子空间都是完全可约的。
  \end{enumerate}
\end{proposition}
