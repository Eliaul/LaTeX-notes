...
\end{proposition}
\begin{proof}
  (1) 任取 $g\in\langle W\rangle$，那么 $g=g_1\cdots g_k$，其中
  $g_i\in W\cup W^{-1}$，那么
  \[
    g=L_{g_1\cdots g_{k-1}}(g_k)\in L_{g_1\cdots g_{k-1}}(W\cup W^{-1})\subseteq \langle W\rangle,
  \]
  由于 $W\cup W^{-1}$ 是开集并且左平移是微分同胚，所以 $L_{g_1\cdots g_{k-1}}(W\cup W^{-1})$
  是开集。这表明 $\langle W\rangle$ 是开集。

  (2) $W$ 连通表明 $W^{-1}$ 连通(求逆是微分同胚)，进而 $W\cup W^{-1}$ 连通(二者有公共点 $e$)。
  若 $\langle W\rangle=U\cup V$，其中 $U,V$ 是 $\langle W\rangle$ 的不相交非空开集。设 $e\in U$，任取 $g\in V$，那么
  $L_g(U)$ 
\end{proof}
