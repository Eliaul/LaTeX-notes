...
令 $G$ 是李群。$G$ 上的协变张量场 $A$ 如果使得任取 $g\in G$ 有 $L_g^*A=A$，
那么说 $A$ 是左不变的。

\begin{proposition}
  令 $G$ 的紧李群配备一个左不变定向。那么 $G$ 有唯一的正定向的
  左不变 $n$-形式 $\omega_G$，满足 $\int_G\omega_G=1$。
\end{proposition}
...
\end{theorem}
\begin{remark}
  $\partial M$ 应当理解为配备诱导的 Stokes 定向，并且
  右端的 $\omega$ 应到理解为 $\iota_{\partial M}^*\omega$。
  如果 $\partial M=\emptyset$，右端理解为零。
\end{remark}
