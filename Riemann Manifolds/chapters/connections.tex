...
为
\begin{equation}\label{eq:euclid directional derivative}
...
得到一个新的向量场 $\wbar\nabla_XY$：
\begin{equation}\label{eq:euclid connection}
...
\begin{equation}\label{eq:tangential directional derivative}
...
目前为止，我们已经研究了联络的一些性质，但是还没有产生出任何联络。
实际上，联络是非常广泛的，我们从一个最简单的例子开始。

\begin{example}[Euclid 联络]
  在 $T \mathbb{R}^n$ 中，定义 \emph{Euclid 联络} $\wbar\nabla$ 为
  \eqref{eq:euclid connection} 式。很容易验证其满足联络的要求，
  并且在标准坐标标架中，其联络系数均为零。
\end{example}

有了 Euclid 联络，就可以构造出一大类联络的例子。

\begin{example}[$\mathbb{R}^n$ 的子流形上的切向联络]\label{exa:tangential connection on submanifold}
  令 $M\subseteq \mathbb{R}^n$ 是嵌入子流形。定义 $TM$ 上的\emph{切向联络}
  $\nabla^\top$ 为
  \[
    \nabla_X^\top Y=\pi^\top\bigl(
      \wbar\nabla_{\wtilde X}\wtilde Y|_M
    \bigr),
  \]
  其中 $\pi^\top$ 是向 $TM$ 的正交投影(\autoref{prop:normal bundle})，
  $\wtilde X$ 和 $\wtilde Y$ 是 $X$ 和 $Y$ 到 $\mathbb{R}^n$ 的某个开子集上的任意延拓。
  因为 $\wbar\nabla_{\wtilde X}\wtilde Y$ 在 $p\in M$ 处的值仅依赖于 $\wtilde X_p=X_p$，
  所以这仅仅是把 $\bigl(\nabla^\top_XY\bigr)_p$ 定义为切向的方向导数 $\nabla_{X_p}^\top Y$，
  就像 \eqref{eq:tangential directional derivative} 式那样。
  
  接下来我们证明，这个联络的定义和 $\wtilde Y$ 的选取无关。
  设 $\wtilde Y_1$ 和 $\wtilde Y_2$ 是 $Y$ 的两个延拓，那么
  $\wtilde Y_1-\wtilde Y_2$ 在 $M$ 上为零。假设 $\wtilde Y_1-\wtilde Y_2=f^iE_i$ 在 $M$ 的某个
  邻域上有定义，那么 $f^i|_M=0$。任取 $p\in M$，记 $v=X_p\in T_pM$，
  那么 $v$ 视为 $T_p\mathbb{R}^n$ 的元素时有 $vf^i=0$，所以 
  $\wbar\nabla_v(\wtilde Y_1-\wtilde Y_2)=0$，即这个联络的定义和 $\wtilde Y$ 的选取无关。

  根据定义很容易验证 $\nabla_X^\top Y$ 在 $X$ 上是 $C^\infty(M)$-线性的，在 $Y$ 上是 $\mathbb{R}$-线性的，
  下面我们验证乘积法则。任取 $f\in C^\infty(M)$，设 $\wtilde f$ 是 $f$ 到 $M$
  的某个邻域上的延拓，那么 $\tilde f\wtilde Y$ 是 $fY$ 在 $M$ 的邻域上的延拓，
  所以
  \begin{align*}
    \nabla_X^\top (fY)&=\pi^\top\left(
      \wbar\nabla_{\wtilde X}\bigl(\tilde f\wtilde Y\bigr)\big|_M
    \right)\\
    &=\pi^\top\left(
      \bigl(\wtilde X\tilde f\bigr)\wtilde Y\big|_M
    \right)+\pi^\top\left(
      \tilde f\wbar \nabla_{\wtilde X}\wtilde Y\big|_M
    \right)\\
    &=(Xf)Y+f\nabla_X^\top Y.
  \end{align*}
  因此 $\nabla^\top$ 是 $TM$ 中的联络。
...
\begin{example}[$\mathbb{S}^2$ 中的联络]\label{exa:connection on sphere}
  考虑单位球面 $\mathbb{S}^2$，对于球面上任意点 $p=(x,y,z)\in \mathbb{S}^2$，
  法向量就是 $p$ 本身。于是对于任意 $w\in \mathbb{R}^3$，其在切空间 $T_p \mathbb{S}^2$
  的投影为：
  \[
    \pi^\top(w)=w-\inn{w,p}p.
  \]
  考虑向量场 $X_p=(0,-z,y)$ 和 $Z_p=(-y,x,0)$，即 $X$ 是绕 $x$ 轴旋转的向量场，$Z$ 是绕 $z$ 轴旋转的向量场。
  由于 $\inn{p,X_p}=\inn{p,Z_p}=0$，所以 $X$ 和 $Z$ 都是 $\mathbb{S}^2$ 上的向量场。
  我们计算联络 $\nabla_{X}^\top Z$。首先我们计算 Euclid 方向导数 $\wbar\nabla_XZ$，有
  \[
    (\wbar\nabla_XZ)_p=\begin{pmatrix}
      0 & -1 & 0 \\
      1 & 0 & 0 \\
      0 & 0 & 0
    ...
    \end{pmatrix}.
  \]
  然后我们把 $\wbar\nabla_XZ$ 投影到 $T_p\mathbb{S}^2$ 上，得到
  \[
    \bigl(\nabla^\top_XZ\bigr)_p=\begin{pmatrix}
      z \\ 0 \\ 0
    \end{pmatrix}-zx\begin{pmatrix}
      x \\ y \\ z
    \end{pmatrix}=\begin{pmatrix}
      z-zx^2 \\ -xyz \\ -x z^2
    \end{pmatrix}.
  ...
  进一步的，我们也可以计算出 $\mathbb{S}^2$ 上切向联络的联络系数。
  使用球坐标系 $(\theta,\phi)$，参数化为
  \[
    r(\theta,\phi)=(\sin\theta\cos\phi,\sin\theta\sin\phi,\cos\theta).
  \]
  于是切空间的两个基为
  \[
    \partial_\theta=(\cos\theta\cos\phi,\cos\theta\sin\phi,-\sin\theta),\quad
    \partial_\phi=(-\sin\theta\sin\phi,\sin\theta\cos\phi,0).
  \]
  这给出了 $\mathbb{S}^2$ 上的局部标架。我们来计算 $\nabla^\top$ 的联络系数。
  这两个向量场可以延拓为
  \[
    \wtilde\partial_\theta=(r\cos\theta\cos\phi,r\cos\theta\sin\phi,-r\sin\theta),\quad
    \wtilde\partial_\phi=(-r\sin\theta\sin\phi,r\sin\theta\cos\phi,0).
  \]

  首先计算 $\wbar\nabla_{\wtilde\partial_\theta}\wtilde\partial_\theta$，有
  \[
    \wbar\nabla_{\wtilde\partial_\theta}\wtilde\partial_\theta=
    \begin{pmatrix}
      -r\sin\theta\cos\phi \\
      -r\sin\theta\sin\phi \\
      -r\cos\theta
    \end{pmatrix},
  \]
  这与 $r(\theta,\varphi)$ 平行，所以 $\nabla^\top_{\partial_\theta}\partial_\theta=0$，即 $\Gamma_{\theta\theta}^\theta=\Gamma_{\theta\theta}^\phi=0$。

  接下来计算 $\wbar\nabla_{\wtilde\partial_\theta}\wtilde\partial_\phi$，有
  \[
    \wbar\nabla_{\wtilde\partial_\theta}\wtilde\partial_\phi=
    \begin{pmatrix}
      -r\cos\theta\sin\phi \\
      r\cos\theta\cos\phi \\ 
      0
  ...
  限制到 $r=1$，再投影得到
  \[
    \nabla^\top_{\partial_\theta}\partial_\phi=\begin{pmatrix}
      -\cos\theta\sin\phi \\
      \cos\theta\cos\phi \\
      0
    \end{pmatrix}=\cot\theta\partial_\phi,
  \]
  这表明 $\Gamma_{\theta\phi}^\theta=0$，$\Gamma_{\theta\phi}^\phi=\cot\theta$。
  不难验证 $\nabla^\top_{\partial_\phi}\partial_\theta=\nabla^\top_{\partial_\theta}\partial_\phi$，所以 $\Gamma_{\phi\theta}^\theta=0$，$\Gamma_{\phi\theta}^\phi=\cot\theta$。

  最后计算 $\wbar\nabla_{\wtilde\partial_\phi}\wtilde\partial_\phi$，有
  \[
    \wbar\nabla_{\wtilde\partial_\phi}\wtilde\partial_\phi=
    \begin{pmatrix}
      -r\sin\theta\cos\phi \\
      -r\sin\theta\sin\phi \\
      0
  ...
  限制到 $r=1$，再投影得到
  \[
    \nabla^\top_{\partial_\phi}\partial_\phi=\begin{pmatrix}
      -\sin\theta\cos\phi+\sin^3\theta \cos\phi\\
      -\sin\theta\sin\phi+\sin^3\theta \sin\phi\\
      \sin^2\theta\cos\theta
    \end{pmatrix}=-\sin\theta\cos\theta\partial_\theta,
  \]
  这表明 $\Gamma_{\phi\phi}^\theta=-\sin\theta\cos\theta$，$\Gamma_{\phi\phi}^\phi=0$。
\end{example}

\begin{example}[$\SO(3)$ 中的联络]\label{exa:connection on SO(3)}
  在 $M(3,\mathbb{R})$ 中，矩阵的内积定义为 $\inn{A,B}=\tr(A^\top B)$，
  即继承向量空间 $\mathbb{R}^9$ 的 Euclid 内积。
  任取 $R\in \SO(3)$，我们知道切空间可以表示为
  \[
    T_R\SO(3)=\big\{RX\,\big|\, X\in \mathfrak{so}(3)\big\}.
  \]
  注意到，对于任意对称矩阵 $S$，有 $\inn{RS,RX}=\inn{S,X}=0$，
  所以 $RS\in N_p\SO(3)$。
  任取一个矩阵 $A\in M(3,\mathbb{R})$，对 $R^\top A$ 分解为
  \[
    R^\top A=\frac{1}{2}\bigl(R^\top A-A^\top R\bigr)+\frac{1}{2}\bigl(R^\top A+A^\top R\bigr),
  \]
  第二部分 $R^\top A+A^\top R$ 是对称矩阵，因此把 $A$ 向 $T_R\SO(3)$ 的投影为
  \[
    \pi^\top(A)=\frac{R}{2}\bigl(R^\top A-A^\top R\bigr).
  \]
  ...
  \]
  其中 $X_1,X_2\in\lie{so}(3)$。我们来计算 $\nabla_{Y}^\top Z$。

  显然，我们可以把 $Y$ 延拓为 $M(3,\mathbb{R})$ 上的向量场 $\wtilde Y_A=AX_1$，同样的，$\wtilde Z_A=AX_2$。
  此时，方向导数
  \[
    \bigl(\wbar\nabla_{\wtilde Y}\wtilde Z\bigr)_A=
    \lim_{t\to 0}\frac{\wtilde Z_{A+t\wtilde Y_A}-\wtilde Z_A}{t}=
    \lim_{t\to 0}\frac{(A+tAX_1)X_2-AX_2}{t}=AX_1X_2.
  \]
  然后我们对 $RX_1X_2$ 进行投影，就有
  \begin{align*}
    \bigl(\nabla_Y^\top Z\bigr)_R&=\pi^\top\bigl(RX_1X_2\bigr)=
    \frac{RX_1X_2-R(RX_1X_2)^\top R}{2}\\
    &=\frac{RX_1X_2-RX_2 X_1}{2}=\frac{R}{2}[X_1,X_2].
  \end{align*}
\end{example}
...
\begin{proof}
  ...
  定义 $D_tV$，现在我们只需要验证这确实满足协变导数的条件。
  
  在一般情况下，我们可以用坐标卡覆盖 $\gamma(I)$
...
\end{corollary}
\begin{proof}
  给定 $p\in M$ 和 $v\in T_pM$，令 $I$ 是所有包含 $0$ 
  且存在给定初始条件的测地线的开区间的并集。根据测地线的存在唯一性，
  这些测地线在它们的公共定义域上相等，所以这定义了一个测地线 $\gamma:I\to M$，
  显然这是唯一的极大测地线。
\end{proof}
...
\begin{example}[$\mathbb{S}^2$ 中的测地线]
  我们利用 \autoref{exa:connection on sphere} 中计算出的 $\mathbb{S}^2$ 上的联络来计算 $\mathbb{S}^2$ 中的测地线。
  在 \autoref{exa:connection on sphere} 中，我们计算出 $\mathbb{S}^2$ 上的联络系数为
  \begin{gather*}
    \Gamma_{\theta\theta}^\theta=0,\ \Gamma_{\theta\theta}^\phi=0,\ \Gamma_{\theta\phi}^\theta=0,\ \Gamma_{\theta\phi}^\phi=\cot\theta,\\
    \Gamma_{\phi\theta}^\theta=0,\ \Gamma_{\phi\theta}^\phi=\cot\theta,\ \Gamma_{\phi\phi}^\theta=-\sin\theta\cos\theta,\ \Gamma_{\phi\phi}^\phi=0.
  \end{gather*}
  假设从赤道上一点 $p=(\pi/2,0)$ (注意我们使用球坐标，第一个分量 $\theta$ 表示余纬度，第二个分量 $\phi$ 表示经度)出发，
  方向沿着赤道方向，即 $\dot\theta(0)=0,\dot\phi(0)=c$。
  根据测地线方程 \eqref{eq:geodesic curve equation}，我们有
  \begin{align*}
    \ddot{\theta}-\sin\theta\cos\theta\,\dot\phi\dot\phi&=0,\\
    \ddot{\phi}+2\cot\theta\,\dot\theta\dot\phi &=0,
  \end{align*}
  猜测 $\theta(t)\equiv \pi/2$，带入得到 $\ddot\phi=0$，所以 
  $\dot\phi(t)\equiv c$，所以 $\phi(t)=ct$。所以曲线 
  $\gamma(t)=\left(\frac{\pi}{2},ct\right)$ 是 $\mathbb{S}^2$ 上的一条测地线，
  根据测地线的存在唯一性，这条测地线就是从 $p$ 出发，初速度沿着赤道方向的极大测地线。
\end{example}

\begin{example}[$\SO(3)$ 中的测地线]\label{exa:geodesics on SO(3)}
  对于 $\SO(3)$，计算联络系数非常繁琐，但是我们可以利用测地线
  的存在唯一性来验证指数映射给出了测地线的形式。给定初始旋转矩阵
  $R_0\in \SO(3)$，假设初速度为 $R_0X\in T_{R_0}\SO(3)$。
  我们断言，以 $R_0$ 为起点初速度为 $R_0 X$ 的测地线为 $\gamma(t)=R_0\exp(tX)$。

  由于 $\gamma'(t)=R_0\exp(tX) X$，根据 \autoref{exa:connection on SO(3)} 中计算出的 $\SO(3)$ 上的联络，我们有
  \[
    D_t\gamma'(t)=\nabla_{\gamma'}^\top\gamma'=
    \frac{R_0\exp(tX)}{2}[X,X]=0,
  \]
  这就表明 $\gamma(t)$ 确实是 $\SO(3)$ 上的一条测地线。根据测地线的存在唯一性，这条测地线就是从 $R_0$ 出发，初速度为 $R_0X$ 的极大测地线。
\end{example}
...
沿 $\gamma$ 平行当且仅当
\begin{equation}\label{eq:parallel transport equation}
...
\begin{example}[$\mathbb{S}^2$ 上的平行移动]
  采用球坐标系。选取一个纬线圈 $\gamma(t)=(\theta_0,t)$，其中 $\theta_0\in (0,\pi)$ 是一个常数
  表示余纬度。
  假设 $t=0$ 时有一个初始向量 $v=(v^\theta,v^\phi)\in T_{\gamma(0)}\mathbb{S}^2$。
  设沿曲线 $\gamma$ 平行的向量场为 $V(t)$。根据平行移动方程 \eqref{eq:parallel transport equation}，
  我们有
  \begin{align*}
    \dot V^\theta&= V^\phi \sin\theta_0\cos\theta_0,\\
    \dot V^\phi&=-V^\theta \cot\theta_0.
  \end{align*}
  把第一个式子求导再将第二个式子代入得到
  \[
  ...
  解得
  \begin{align*}
    V^\theta(t)&=v^\theta \cos(t\cos\theta_0)+v^\phi\sin\theta_0\sin(t\cos\theta_0),\\
    V^\phi(t)&=-\frac{v^\theta}{\sin\theta_0}\sin(t\cos\theta_0)+v^\phi\cos(t\cos\theta_0).
  \end{align*}
  所以平行移动映射为
  \[
     P_{0t}^\gamma(v)=
     \begin{pmatrix}
      \cos(t\cos\theta_0) & \sin\theta_0\sin(t\cos\theta_0)\\[2mm]
      -\dfrac{\sin(t\cos\theta_0)}{\sin\theta_0} & \cos(t\cos\theta_0)
     \end{pmatrix}
     \begin{pmatrix}
      v^\theta\\
      v^\phi
...
\begin{example}[$\SO(3)$ 上的平行移动]
  任取 $R_0\in \SO(3)$，以及 $R_0X\in T_{R_0}\SO(3)$，
  根据 \autoref{exa:geodesics on SO(3)}，以 $R_0$ 为起点初速度为 $R_0X$ 的测地线为 $\gamma(t)=R_0\exp(tX)$。
  我们计算沿 $\gamma$ 的平行移动。

  假设沿曲线 $\gamma$ 平行的向量场为 $V(t)$，由于 $V(t)\in T_{\gamma(t)}\SO(3)$，
  所以可设 $V(t)=\gamma(t) Y(t)$，其中 $Y(t)\in\lie{so}(3)$。
  那么在 Euclid 空间中的方向导数为
  \[
    \wbar\nabla_{\gamma'}V=\dot V(t)=
    \gamma(t)XY(t)+\gamma(t)\dot Y(t).
  \]
  向 $T_{\gamma(t)}\SO(3)$ 投影，所以
  \[
    D_tV(t)=\gamma(t)\left(
      \frac{1}{2}[X,Y(t)]+\dot Y(t)
    \right).
  \]
  所以 $V$ 沿 $\gamma$ 平行当且仅当
  \[
  ...
  \]
  由于 $Y(t)$ 和 $X$ 都是反对称矩阵，所以可设 $Y(t)=(\eta(t))^\wedge$
  和 $X=\omega^\wedge$，其中 $\eta(t),\omega\in \mathbb{R}^3$，$\wedge:\mathbb{R}^3\to \lie{so}(3)$ 是李代数同构。
  那么 $V$ 沿 $\gamma$ 平行当且仅当
  \[
  ...
  \[
    V(t)=\gamma(t)\left(
      \exp\left(-\frac{tX}{2}\right)\eta(0)
    \right)^\wedge.
  \]
\end{example}

把平行移动拓展到分段光滑曲线上也是有用的。给定一个分段光滑曲线
$\gamma:[a,b]\to M$ 和映射 $V:[a,b]\to TM$，如果 $V(t)\in T_{\gamma(t)}M$ 且
$V$ 是连续的并且在 $\gamma$ 的划分 $(a_0,\dots,a_k)$ 上使得 
$V|_{[a_{i-1},a_i]}$ 是光滑的，那么我们说 $V$ 是\emph{沿 $\gamma$ 的分段光滑向量场}。
如果 $V$ 在每个区间 $[a_{i-1},a_i]$ 上都是沿 $\gamma|_{[a_{i-1},a_i]}$ 平行的，那么我们说 $V$ 是\emph{沿 $\gamma$ 的平行的}。

给定 $T_{\gamma(t_0)}M$ 的任意基 $(b_1,\dots,b_n)$，我们可以把每个向量
$b_i$ 沿 $\gamma$ 平行移动，从而得到 $n$ 个沿 $\gamma$ 平行的向量场
$(E_1,\dots,E_n)$。因为每个平行移动都是同构，所以在每个点 $\gamma(t)$
处，向量 $(E_i(t))$ 都构成 $T_{\gamma(t)}M$ 的一组基。这样的 $n$ 个
沿 $\gamma$ 平行的向量场被称为\emph{沿 $\gamma$ 的平行标架}。每个沿 $\gamma$
光滑(或者分段光滑)的向量场都可以在这个标架中表示为 $V(t)=V^i(t)E_i(t)$，
那么根据沿曲线的协变导数的性质，并且因为 $E_i$ 是沿 $\gamma$ 平行的，所以
\begin{equation}
  D_tV(t)=\dot V^i(t)E_i(t).
\end{equation}
这意味着一个向量场沿 $\gamma$ 平行当且仅当其相对于标架 $(E_i)$ 的分量是常值函数。

平行移动映射是连接切空间的手段。下面的定理和推论表明平行移动确定了
沿曲线的协变导数，因此也确定了联络本身。

\begin{theorem}[平行移动确定协变导数]
  令 $M$ 是带边或者无边光滑流形，$\nabla$ 是 $TM$ 中的联络。
  假设 $\gamma:I\to M$ 是光滑曲线，$V$ 是沿 $\gamma$ 的光滑向量场。
  对于每个 $\gamma_0\in I$，有
  \begin{equation}
...
\end{theorem}
\begin{proof}
  令 $(E_i)$ 是沿 $\gamma$ 的平行标架，记 $V(t)=V^i(t)E_i(t)$。
  此时 $D_tV(t)=\dot V^i(t)E_i(t)$。

  另一方面，对于每个 $t_1\in I$，向量 $V(t_1)$ 沿 $\gamma$
  的平行移动是沿 $\gamma$ 的常系数向量场 $W(t)=V^i(t_1)E_i(t)$，
  所以 $P_{t_1t_0}^\gamma V(t_1)=V^i(t_1)E_i(t_0)$，因此
  \[
    \lim_{t_1\to t_0}\frac{P_{t_1t_0}^\gamma V(t_1)-V(t_0)}{t_1-t_0}=
    \dot V^i(t_0)E_i(t_0)=D_tV(t_0).\qedhere
  \]
\end{proof}

\begin{corollary}[平行移动确定联络]
  令 $M$ 是带边或者无边光滑流形，$\nabla$ 是 $TM$ 中的联络。
  假设 $X,Y$ 是 $M$ 上的光滑向量场。对于 $p\in M$，有
  \begin{equation}
    \nabla_XY|_p=\lim_{h\to 0}\frac{P_{h0}^\gamma Y_{\gamma(h)}-Y_{p}}{h},
  \end{equation}
  其中 $\gamma:I\to M$ 是任意满足 $\gamma(0)=p$ 和 $\gamma'(0)=X_p$ 的光滑曲线。
\end{corollary}
\begin{proof}
  由于 $\nabla_XY|_p$ 只与 $X_p$ 和 $Y$ 在 $\gamma(I)$ 上的取值有关，
  所以我们可以定义沿 $\gamma$ 的光滑向量场 $V(t)=Y_{\gamma(t)}$，那么 $V(0)=Y_p$，所以
  \[
    \nabla_XY|_p=D_tV(0)=\lim_{h\to 0}\frac{P_{h0}^\gamma Y_{\gamma(h)}-Y_p}{h}.\qedhere
  \]
\end{proof}

$M$ 上的光滑向量场或者张量场被称为(相对于 $\nabla$)\emph{平行的}，如果
其沿 $M$ 中的每条光滑曲线都是平行的。

\begin{proposition}
  设 $M$ 是带边或者无边光滑流形，$\nabla$ 是 $TM$ 中的联络，
  $A$ 是 $M$ 上的光滑向量场或者张量场。那么 $A$ 是平行的当且仅当 $\nabla A\equiv 0$。
\end{proposition}
