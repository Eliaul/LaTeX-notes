...
\begin{proposition}\label{prop:gaussian independence}
...
\end{theorem}
\begin{proof}
  令 $\widehat X=\sum_{j=1}^n\lambda_jY_j$ 是 $X$ 在由 $Y_1,\dots,Y_n$ 张成的
  向量空间上的正交投影。那么，对于每个 $j\in\{1,\dots,n\}$，有
  \[
    \cov(X-\widehat X,Y_j)=\mathbb{E}[(X-\widehat X)Y_j]=0.
  \]
  因为 $(Y_1,\dots,Y_n,X-\widehat X)$ 是 $Y_1,\dots,Y_n,X$ 的线性组合，
  所以 $(Y_1,\dots,Y_n,X-\widehat X)$ 也是高斯向量，于是 \autoref{prop:gaussian independence}
  表明 $X-\widehat X$ 和 $(Y_1,\dots,Y_n)$ 独立。因此，根据条件期望的性质，有
  \[
    \mathbb{E}[X|Y_1,\dots,Y_n]=\mathbb{E}[X-\widehat X|Y_1,\dots,Y_n]+\widehat X
    =\mathbb{E}[X-\widehat X]+\widehat X=\widehat X.
  \]

  对于后面的结论，令 $Z=X-\widehat X$，那么 $Z$ 与  $(Y_1,\dots,Y_n)$ 独立且
  服从分布 $N(0,\sigma^2)$。那么
  \begin{align*}
    \mathbb{E}[h(X)|Y_1,\dots,Y_n]&=\mathbb{E}\biggl[
      h\biggl(
        \sum_{j=1}^n \lambda_j Y_j +Z
      \biggr)\bigg| Y_1,\dots,Y_n 
    \biggr]\\
    &=\int h\biggl(
        \sum_{j=1}^n \lambda_j Y_j +z
      \biggr)p_{Z|Y_1,\dots,Y_n}(z|Y_1,\dots,Y_n)\d z\\
    &=\int h\biggl(
        \sum_{j=1}^n \lambda_j Y_j +z
      \biggr)p_{Z}(z)\d z,
  \end{align*}
  此时 $p_Z(z)$ 就是 $N(0,\sigma^2)$ 的密度函数 $q_{0,\sigma^2}(z)$，然后在进行简单的换元，
  这就完成了证明。
\end{proof}
