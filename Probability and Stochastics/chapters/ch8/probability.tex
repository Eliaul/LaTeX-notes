...
\paragraph{分布律的刻画} 根据 \autoref{coro:uniqueness of measure}，$\mathbb{R}^d$
上的概率测度 $\mu$ 完全由其在开矩形上的值确定，也即由所有 $\mu((a_1,b_1)\times\cdots\times (a_d,b_d))$
确定 (当然，也可以把区间换成闭区间，甚至要求所有的 $a_i,b_i$ 都是有理数)。因为开矩形
上的示性函数是一列紧支连续函数的递增极限，所以概率测度 $\mu$ 也可以由所有 $\int\varphi(x)\mu(\d x)$
的值确定，其中 $\varphi\in C_c(\mathbb{R}^d)$ 是从 $\mathbb{R}^d$ 到 $\mathbb{R}$ 的紧支连续函数。
特别地，对于值在 $\mathbb{R}^d$ 中的随机向量 $X=(X_1,\dots,X_d)$，分布律 $\mathbb{P}_X$
由所有
\[
  \mathbb{P}_X((a_1,b_1)\times\cdots\times (a_d,b_d))=\mathbb{P}
  (a_1<X_1<b_1,\dots,a_d<X_d<b_d)
\]
确定，其中 $a_1<b_1,\dots,a_d<b_d$。或者，由所有
\[
  \int_{\mathbb{R}^d}\varphi(x) \mathbb{P}_X(\d x)=\mathbb{E}[\varphi(X)]
\]
确定，其中 $\varphi\in C_c(\mathbb{R}^d)$。
...
\]
其中 $f:E\to \mathbb{R}$ 属于某个足够大的函数类(如 $E=\mathbb{R}^d$ 时 $f$ 取变所有紧支的连续函数)，
此时就一定有 $\mathbb{P}_X=\nu$。下面的命题应用了这样的思想。
...
\begin{proposition}
  ...
  这里 $\cov(X,Y)$ 表示行向量 $(\cov(X,Y_j))_{1\leq j\leq n}$，此时
  \[
    Z=\mathbb{E}[X]+\cov(X,Y)K_Y^{-1}(Y-\mathbb{E}[Y]).
  \]
\end{proposition}
\begin{proof}
  令 $H$ 是 $1,Y_1,\dots,Y_n$ 张成的 $L^2(\Omega,\mathcal{A},\mathbb{P})$
  的闭子空间，或者等价的说，由 $1,Y_1-\mathbb{E}[Y_1],\dots,Y_n-\mathbb{E}[Y_n]$
  张成的闭子空间。根据 Hilbert 空间的投影定理，我们知道使得 $\Vert X-U\Vert_2$
  最小的 $U\in H$ 是 $X$ 在 $H$ 上的正交投影。记这个投影为 $Z$，设
  \[
    Z=\alpha_0+\sum_{j=1}^n \alpha_j (Y_j- \mathbb{E}[Y_j]).
  \] 
  那么必须有 $X-Z$ 与空间 $H$ 正交，这当且仅当
  \[
    \mathbb{E}[(X-Z)\times 1] =0,
  \]  
  以及对于每个 $1\leq k\leq n$，有
  \[
    \mathbb{E}[(X-Z)(Y_k-\mathbb{E}[Y_k])]=0.
  \]
  由于
  \[
    \mathbb{E}[X-Z]=\mathbb{E}[X]-\alpha_0=0,
  \]
  所以 $\alpha_0=\mathbb{E}[X]$。此外，对于每个 $1\leq k\leq n$，有
  \[
    \mathbb{E}[(X-Z)(Y_k-\mathbb{E}[Y_k])]=
    \cov(X,Y_k)-\sum_{j=1}^n \alpha_j \cov(Y_j,Y_k)=0.
  \]
  这就得到了系数 $\alpha_j$ 满足的线性方程组。
\end{proof}
...
\begin{lemma}\label{lemma:characteristic function of normal distribution}
...
\begin{theorem}\label{thm:characteristic function determines distribution}
...
\begin{proof}
  ...
  构成的空间。回顾 $\mathbb{R}$ 上的概率测度 $\nu$ 由所有 $\int \varphi(x)\nu(\d x)$
  刻画，其中 $\varphi\in C_c(\mathbb{R})\subseteq C_b(\mathbb{R})$。我们证明
  下面两点：
  \begin{enumerate}
    \item $\mu_\sigma$ 被 $\hat\mu$ 确定。
    \item 对于每个 $\varphi\in C_b(\mathbb{R})$，当 $\sigma\to 0$ 时有
    $\int\varphi(x)\mu_\sigma(\d x)\to\int\varphi(x)\mu(\d x)$。
  \end{enumerate}

  根据 \autoref{lemma:characteristic function of normal distribution}，对于任意 $x\in \mathbb{R}$，有
  \[
    \sigma\sqrt{2\pi}g_\sigma(x)=\exp\left(-\frac{x^2}{2\sigma^2}\right)
    =\int_{\mathbb{R}}e^{i\xi x}g_{1/\sigma}(\xi) \d\xi.
  \]
  因此，有
  \begin{align*}
    f_\sigma(x)=\int_{\mathbb{R}}g_\sigma(x-y)\mu(\d y)&=
    \bigl(\sigma\sqrt{2\pi}\bigr)^{-1}\int_{\mathbb{R}}
    \left(
      \int_{\mathbb{R}}e^{i\xi (x-y)}g_{1/\sigma}(\xi)\d\xi
    \right) \mu(\d y)\\
    &=\bigl(\sigma\sqrt{2\pi}\bigr)^{-1}\int_{\mathbb{R}}e^{i\xi x}
    g_{1/\sigma}(\xi)\left(
      \int_{\mathbb{R}}e^{-i\xi y}\mu(\d y)
    \right)\d\xi\\
    &=\bigl(\sigma\sqrt{2\pi}\bigr)^{-1}\int_{\mathbb{R}}e^{i\xi x}g_{1/\sigma}(\xi)
    \hat\mu(-\xi)\d \xi.
  \end{align*}
  注意我们使用了 Fubini-Tonelli 定理来交换积分次序。这就表明 $f_\sigma$
  完全由 $\hat\mu$ 确定，所以 $\mu_\sigma$ 也由 $\hat\mu$ 确定。

  下面证明 (2)。对于每个 $\varphi\in C_b(\mathbb{R})$，有
  \begin{align*}
    \int\varphi(x)\mu_\sigma(\d x)=\int \varphi(x)\left(
      \int g_\sigma(x-y)\mu(\d y)
    \right) \d x&=\int\left(
      \int \varphi(x)g_\sigma(x-y)\d x
    \right) \mu(\d y)\\
    &=\int g_\sigma *\varphi(y)\mu(\d y).
  \end{align*}
  这里再次使用了 Fubini-Tonelli 定理来交换积分次序。注意到
  \begin{gather*}
    \int g_\sigma(x) \d x=1,\\
    \lim_{\sigma\to 0}\int_{\{|x|>\varepsilon\}}g_\sigma(x)\d x=0,\quad 
    \forall\varepsilon>0.
  \end{gather*}
  这表明对于任意 $y\in \mathbb{R}$，有
  \[
    \lim_{\sigma\to 0}g_\sigma*\varphi(y)=\varphi(y).
  \]
  根据控制收敛，注意到 $|g_\sigma*\varphi|\leq \sup\{|\varphi(x)|\,|\, x\in \mathbb{R}\}$，
  我们有
  \[
    \lim_{\sigma\to 0}\int\varphi(x)\mu_\sigma(\d x)=\lim_{\sigma\to 0}
    \int g_\sigma*\varphi(y)\mu(\d y)=\int\varphi(x)\mu(\d x).
  \]
  这就证明了 (2)。
  ...
  \]
  即可。此时，对于 $\xi=(\xi_1,\dots,\xi_d)\in \mathbb{R}^d$ 有
  \[
    \int_{\mathbb{R}^d} g_\sigma^{(d)}(x)e^{i\xi\cdot x}\d x=
    \prod_{j=1}^d \int_{\mathbb{R}} g_\sigma(x_j)e^{i\xi_jx_j}\d x_j=
