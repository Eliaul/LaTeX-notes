...
\newcommand{\ulim}[1][]{\lim_{#1}\mathord{\uparrow}}
\newcommand{\dlim}[1][]{\lim_{#1}\mathord{\downarrow}}
...
换句话说，$p$ 是 $\mathbb{P}_X$ 相对于 Lebesgue 测度的密度。
...
下面的命题是 \autoref{prop:change variable} 的特例，其十分重要，所以我们再次叙述一遍。

\begin{proposition}
  令 $X$ 是值在 $(E,\mathcal{E})$ 中的随机变量，对于任意可测函数 $f:E\to [0,\infty]$，
  我们有
  \[
    \mathbb{E}[f(X)]=\int_E f(x) \mathbb{P}_X(\d x).
  \]
\end{proposition}
